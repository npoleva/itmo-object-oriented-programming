...
{
    [Fact]
    public void TryBuildComputerShouldReturnSuccessfulResultWithoutMessages()
    {
        ...
        ComputerComponentRecord videoCardRecord = new VideoCardRecord(
            "KFA2 GeForce RTX 3060 CORE (LHR)",
            258,
            126,
            12,
            4,
        ...
        Assert.Equal(Outcome.Valid, result.BuildingOutcome);

        Assert.All(result.Messages, message =>
            Assert.Equal(MessagesEnumeration.None, message));
    }
}